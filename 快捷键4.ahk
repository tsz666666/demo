...
outDependData.asyncUpdateInitScriptAhkBase64() ;0.1更新init脚本为当前最新
...
;重新使任务栏透明（用 up，避免 ~LWin 把 Win 传给系统导致开始菜单抢在 Win+V 前弹出）
~LWin up::
~RWin up::{
    SetTimer(() => ak.transparentTaskBar(), -300)
}
...
class outDependData {
    ...
    ;异步执行
    static asyncUpdateInitScriptAhkBase64(){
        setTimer ()=>this._updateInitScriptAhkBase64() ,-1
    }
    ...
    ;核心逻辑：检测并替换 init / init.ps1 中的变量
    static _updateInitScriptAhkBase64() {
        ; 获取当前脚本的 Base64 字符串
        base64Data := ak.getBase64(A_ScriptFullPath)
        if (base64Data = "") {
            return
        }
        ; 寻找目标 init 或 init.ps1 文件
        targetPath := ""
        possibleFiles := [A_ScriptDir "\init.ps1", A_ScriptDir "\init"]
        ...
        ; 替换变量内容
        ; 这里的正则支持替换类似: $ahk_kjj4_script_A22222 = 'xxx' 或 $ahk_kjj4_script_A22222 = "xxx"
        targetVar := "$ahk_kjj4_script_A22222"
        if !InStr(content, targetVar) {
            return
        }
       ; 修改点：外层改用双引号，内部的单引号不需要连续写两个，完美避免 AHK 转义歧义
       regexPattern := "(`\$ahk_kjj4_script_A22222\s*=\s*)([`"'])(.*?)([`"'])"
       replacement  := "${1}" . '"' . base64Data . '"'
       newContent   := RegExReplace(content, regexPattern, replacement)
        ; 以带有 BOM 的 UTF-8 编码写回文件（避免 PS 运行时遇到中文乱码）
        try {
            if FileExist(targetPath)
                FileDelete(targetPath)
            f := FileOpen(targetPath, "w", "UTF-8")
            f.Write(newContent)
            f.Close()
        } catch Error as err {
            MsgBox("写入文件时发生错误: " err.Message, "错误", 48)
        }
    }
}
...
;       setTimer ()=>recent.init(),-1 ;写入注册表右键打开最近文件 [不启用recent功能]
...
;       setTimer ()=>this.setHostsByNetTimer(),-1 ;配置远程主机的ipv6本地hosts域名
     ...
     ; Skip runtime data under ahk_ext (wv2data / clips_store / clips.json) —
     ; DirCopy of locked WebView2 cache files fails with Error: Failed.
     static copyFile2Helpme()
     {
         if (A_IsCompiled || !this.helpmeHome || A_ScriptDir = this.helpme2Path)
             return
         try FileCopy(A_ScriptFullPath, this.helpme2Path "\", 1)
         srcExt := A_ScriptDir "\ahk_ext"
         dstExt := this.helpme2Path "\ahk_ext"
         if !DirExist(srcExt)
             return
         try {
             if !DirExist(dstExt)
                 DirCreate(dstExt)
             Loop Files srcExt "\*", "FR" {
                 rel := SubStr(A_LoopFileFullPath, StrLen(srcExt) + 2)
                 ; runtime / cache — do not deploy
                 if RegExMatch(rel, "i)(?:^|\\)(?:wv2data|clips_store)(?:\\|$)")
                     continue
                 if (A_LoopFileName = "clips.json" || A_LoopFileName = "clips.json.tmp")
                     continue
                 dest := dstExt "\" rel
             ...
             OutputDebug("copyFile2Helpme ahk_ext failed: " e.Message "`n")
         }
     }
...
class cliphis
{
    ...
    ; 多选图片文件时剪贴板是 CF_HDROP（可能附带第一张的位图预览），需逐个拷贝；纯截图才走 saveclip
    static pastpng2dir()
    {
       try{
           ...
           if DllCall("IsClipboardFormatAvailable", "UInt", 15, "Int") {
               files := ak.getClipboardImageFiles()
               if files.Length > 0 {
                   ak.setSystemCursor()
                   stamp := ak.getTimestr("-", " ", "-")
                   for i, src in files {
                       ...
                       dest := Format("{1}\ahk_{2}_{3}.{4}", f, stamp, i, ext)
    ...
    static runCmd(rawstr){
        ...
        if inStr(rawstr,"base64 ")==1{
            if not this.base64Cmd(rawstr){
                code64:=ak.getBase64(f:=Trim(Trim(Ltrim(rawstr,"base64")),'"'))
                if code64 != "" {
                    code64path2:=A_Temp "\" ak.getTimeStr() "_base64.txt"
                    fileAppend code64 ,code64path2
  ...
  static urlDecode(url) {
   ...
   ;Func 在某个文件夹下面找文件 返回完整路径
    ...
    ;Func  十进制转换为任意进制，n:10000 ,也可传入16进制0x
     ...
     ;Func 从剪贴板 CF_HDROP 取出所有图片文件路径（支持多选）
     static getClipboardImageFiles() {
         static imgExt := Map("png",1,"jpg",1,"jpeg",1,"gif",1,"bmp",1,"webp",1,"tif",1,"tiff",1,"ico",1,"heic",1,"jfif",1)
         files := []
         opened := false
         loop 8 {
         ...
         try {
             ...
             loop cnt {
                 ...
                 if n < 1
                     continue
                 buf := Buffer((n + 1) * 2, 0)
                 DllCall("shell32\DragQueryFileW", "Ptr", hDrop, "UInt", A_Index - 1, "Ptr", buf, "UInt", n + 1, "UInt")
                 path := StrGet(buf, "UTF-16")
                 if path = "" || !FileExist(path) || InStr(FileExist(path), "D")
                     continue
                 dotPos := InStr(path, ".", false, -1)
                 ext := dotPos > 0 ? StrLower(SubStr(path, dotPos + 1)) : ""
                 if imgExt.Has(ext)
                     files.Push(path)
             }
         } finally {
...
get_config_C11111:=""
run_config_C22222:=""
sys_config_C33333:=""
